      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: '12px', marginBottom: '16px' }}>
        {[
          ...
          <div key={i} style={{ background: '#13161F', border: '1px solid #252836', borderRadius: '14px', padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '22px', fontWeight: 700, ...mono }}><ContadorAnimado valor={k.valorNum} cor={k.cor} formatador={fmtR} /></div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showPagtoMenu && (<div style={{ position:'absolute', top:'100%', left:0, zIndex:100, background:'#1A1D2A', border:'1px solid #353849', borderRadius:6, padding:'4px 0', minWidth:'160px', maxHeight:'260px', overflowY:'auto' }}>
                  ...
                  <div style={{ borderTop:'1px solid #353849', margin:'4px 0', padding:'4px 12px' }}>
                    ...
                    <button onClick={() => setShowPagtoMenu(false)} style={{ fontSize:'11px', color:'#34D399', background:'none', border:'none', cursor:'pointer', marginLeft:8 }}>Fechar</button>
                  </div>
                </div>)}
              </div>
              <div style={{ position: 'relative' }}>
                ...
                {showContbMenu && (<div style={{ position:'absolute', top:'100%', left:0, zIndex:100, background:'#1A1D2A', border:'1px solid #353849', borderRadius:6, padding:'4px 0', minWidth:'170px', maxHeight:'260px', overflowY:'auto' }}>
                  ...
                  <div style={{ borderTop:'1px solid #353849', margin:'4px 0', padding:'4px 12px' }}>
                    ...
                    <button onClick={() => setShowContbMenu(false)} style={{ fontSize:'11px', color:'#34D399', background:'none', border:'none', cursor:'pointer', marginLeft:8 }}>Fechar</button>
                  </div>
                </div>)}
              </div>
              <div style={{ position: 'relative' }}>
                ...
                {showStatusMenu && (<div style={{ position:'absolute', top:'100%', left:0, zIndex:100, background:'#1A1D2A', border:'1px solid #353849', borderRadius:6, padding:'4px 0', minWidth:'200px' }}>
                  ...
                  <div style={{ borderTop:'1px solid #353849', margin:'4px 0', padding:'4px 12px' }}>
                    <button onClick={() => { setFiltroStatus([]); setShowStatusMenu(false) }} style={{ fontSize:'11px', color:'#7B82A0', background:'none', border:'none', cursor:'pointer' }}>Limpar</button><button onClick={() => setShowStatusMenu(false)} style={{ fontSize:'11px', color:'#34D399', background:'none', border:'none', cursor:'pointer', marginLeft:8 }}>Fechar</button>
                  </div>
                </div>)}
              </div>
            </div>
            ...
            <table style={{ width: isMobile ? 'max-content' : '100%', minWidth: isMobile ? '900px' : undefined, borderCollapse: 'collapse', fontSize: '12px', tableLayout: 'fixed' }}>
              <thead>
                <tr style={{ background: '#1A1D2A' }}>
                  ...
                  <th style={{ fontSize: '10px', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '1px', color: '#7B82A0', borderBottom: '1px solid #252836', whiteSpace: 'nowrap' as const, padding: '8px 12px' }}>Destinatário</th>
                  <th style={{ fontSize: '10px', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '1px', color: '#7B82A0', borderBottom: '1px solid #252836', whiteSpace: 'nowrap' as const, padding: '8px 12px' }}>CNPJ Dest.</th>
                  <th style={{ fontSize: '10px', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '1px', color: '#7B82A0', borderBottom: '1px solid #252836', whiteSpace: 'nowrap' as const, padding: '8px 12px', textAlign: 'right' as const }}>Valor NF</th>
                  <th style={{ fontSize: '10px', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '1px', color: '#7B82A0', borderBottom: '1px solid #252836', whiteSpace: 'nowrap' as const, padding: '8px 12px', textAlign: 'right' as const }}>Dt. Emissão</th>
                  ...
                  <th style={{ fontSize: '10px', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '1px', color: '#7B82A0', borderBottom: '1px solid #252836', whiteSpace: 'nowrap' as const, padding: '8px 12px' }}></th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <React.Fragment key={r.numero_nf}>
                      ...
                      <tr style={rowStyle}>
                        ...
                        <td style={tdBase({ textAlign: 'right', ...mono, fontSize: '11px' })}>
                          {(() => {
                            const dtContb = temHistorico ? (lista[0]?.data_contabilizacao || '') : (r.data_contabilizacao || '')
                        ...
                        return (
                          <>
                            <tr key={r.numero_nf + '-pg-' + idx} style={{ background: 'rgba(79,142,247,0.04)', borderLeft: '3px solid #4F8EF7' }}>
                              ...
                              <td style={tdBase({ color: '#4A5070', ...mono, fontSize: '11px', whiteSpace: 'nowrap' })}>{fmtCNPJ(r.cnpj_dest)}</td>
                              <td style={tdBase({ textAlign: 'right', fontWeight: 400, color: '#4A5070', ...mono, fontSize: '11px' })}>{r.valor_nf ? fmtR(parseFloat(r.valor_nf)) : '—'}</td>
                              <td style={tdBase({ textAlign: 'right', color: '#7B82A0', ...mono, fontSize: '11px' })}>{r.data_emissao || '�'}</td>
                              ...
                              <td style={tdBase({ textAlign: 'right', ...mono, fontSize: '11px' })}>
                                {(() => {
                                  const dtContbPg = pg.data_contabilizacao || ''
                      ...
                      {mostrarProxima && filtroMesPagto.length === 0 && (
                        <>
                          <tr key={r.numero_nf + '-prox'} style={{ background: 'rgba(248,113,113,0.03)', borderLeft: '3px solid #F87171' }}>
                            ...
                            <td style={tdSm({ color: '#4A5070', ...mono, fontSize: '11px', whiteSpace: 'nowrap' })}>{fmtCNPJ(r.cnpj_dest)}</td>
                            <td style={tdSm({ textAlign: 'right', color: '#4A5070', ...mono, fontSize: '11px' })}>{r.valor_nf ? fmtR(parseFloat(r.valor_nf)) : '—'}</td>
