                        ...
                        return (
                          <>
                            <tr key={r.numero_nf + '-pg-' + idx} style={{ background: 'rgba(79,142,247,0.04)', borderLeft: '3px solid #4F8EF7' }}>
                              ...
                              <td style={tdSm({ textAlign: 'center', whiteSpace: 'nowrap' })}>{isVenda && <button onClick={async (e) => { e.preventDefault(); e.stopPropagation(); const val = !(ajustadosPg[pg.id] || false); setAjustadosPg(prev => ({...prev, [pg.id]: val})); await api.put('/notas/ajustado/' + r.id, { ajustado: val }) }} style={{ width: 14, height: 14, borderRadius: 3, border: '1px solid #4A5070', background: ajustadosPg[pg.id] ? '#34D399' : '#2A2D3E', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{ajustadosPg[pg.id] && <span style={{ color: '#0D0F17', fontSize: 10, fontWeight: 700 }}>✓</span>}</button>}</td>
                              <td style={tdBase({ textAlign: 'center', whiteSpace: 'nowrap' })}>
