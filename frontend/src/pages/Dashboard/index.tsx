...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const PAD = { top: 60, right: 16, bottom: 28, left: 52 }
    const W = canvas.offsetWidth
    const H = canvas.offsetHeight
    canvas.width = W
    canvas.height = H
    const cW = W - PAD.left - PAD.right
    const cH = (H - PAD.top - PAD.bottom) * 0.88
    ...
    const refW = Math.min(cW, Math.max(560, nBars*56))
    const groupW = refW / nBars
        const barW = Math.max(32, Math.min(56, Math.floor(560/nBars)-10))
    ...
    function draw(ts: number) {
        ...
        if (stack) {
          ...
          barSeries.forEach((s, si) => {
            s.data.forEach((v, i) => {
              ...
              const lp = Math.max(0, Math.min(1, (ease - delay) / 0.6))
              const bH = v / maxV * cH * lp
              const x = PAD.left + groupW * i + groupW / 2 + groupOffset + si * barW - barW / 2
              const sd=(s as any).stack
              if(sd&&sd[i]){
                let yB=PAD.top+cH
                const hS=sd[i].six/maxV*cH*lp;yB-=hS
                ctx.fillStyle='#4F8EF7CC';ctx.beginPath()
                if(ctx.roundRect)ctx.roundRect(x,yB,barW,Math.max(0,hS),[0,0,2,2]);else ctx.rect(x,yB,barW,Math.max(0,hS));ctx.fill()
                const hE=sd[i].enova/maxV*cH*lp;yB-=hE
                ctx.fillStyle='#34D399CC';ctx.beginPath();ctx.rect(x,yB,barW,Math.max(0,hE));ctx.fill()
                const hC=sd[i].cm/maxV*cH*lp;yB-=hC
                ctx.fillStyle='#F472B6CC';ctx.beginPath()
                if(ctx.roundRect)ctx.roundRect(x,yB,barW,Math.max(0,hC),[2,2,0,0]);else ctx.rect(x,yB,barW,Math.max(0,hC));ctx.fill()
                const tot2=sd[i].six+sd[i].enova+sd[i].cm
                const yFinal=PAD.top+cH-(tot2/maxV*cH)
                if(tot2>0){ctx.save();ctx.fillStyle='#E8EAF0';ctx.font='bold 10px monospace';ctx.textAlign='center';ctx.fillText(fmtK2(tot2),x+barW/2,yFinal<PAD.top+14?PAD.top+14:yFinal-5);ctx.restore()}
                const totH=hS+hE+hC
                if(totH>8&&lp>0.9){const tot=sd[i].six+sd[i].enova+sd[i].cm;const yTop=PAD.top+cH-(tot/maxV*cH);ctx.save();ctx.fillStyle='#E8EAF0';ctx.font='bold 10px monospace';ctx.textAlign='center';ctx.fillText(fmtK2(tot),x+barW/2,Math.max(PAD.top+10,yTop-4));ctx.restore()}
              }else{
                ctx.fillStyle=s.color+'CC';ctx.beginPath()
                if(ctx.roundRect)ctx.roundRect(x,PAD.top+cH-bH,barW,Math.max(0,bH),2);else ctx.rect(x,PAD.top+cH-bH,barW,Math.max(0,bH));ctx.fill()}
            })
          })
        }
      ...
      if (progress < 1) animRef.current = requestAnimationFrame(draw)
        ...
        barSeries.forEach((s)=>{s.data.forEach((v,i)=>{const sd=(s as any).stack;if(sd&&sd[i]){const tot=sd[i].six+sd[i].enova+sd[i].cm;if(tot>0){const xx=PAD.left+groupW*i+groupW/2;const yT=PAD.top+cH-(tot/maxV*cH);ctx.save();ctx.fillStyle="#E8EAF0";ctx.font="bold 10px monospace";ctx.textAlign="center";ctx.fillText(fmtK2(tot),xx,Math.max(PAD.top+12,yT-4));ctx.restore()}}})});
      }
    }
