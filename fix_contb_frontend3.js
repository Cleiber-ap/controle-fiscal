...
const idxState = c.indexOf(anchorState);
if (idxState === -1) { console.log("FALHOU: anchorState"); ok = false; }
else {
  const insertState = `
  const [editandoContb, setEditandoContb] = useState<string | null>(null)
  const [editContbVal, setEditContbVal] = useState("")`;
  c = c.replace(anchorState, anchorState + insertState);
}

// 2. Novo cabecalho <th> apos Dt. Pagto
const anchorTh = ">Dt. Pagto</th>";
const idxTh = c.indexOf(anchorTh);
if (idxTh === -1) { console.log("FALHOU: anchorTh"); ok = false; }
else {
  const novoTh = `
...
const idxTd = c.indexOf(anchorTd);
if (idxTd === -1) { console.log("FALHOU: anchorTd"); ok = false; }
else {
  const novoTd = `
