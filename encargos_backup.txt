...
function getTodosOsFeriados(ano: number, custom: any[] = []) {
  ...
  const fixos = [
    {dia:1,mes:1,descricao:'Confraterniza��o Universal',tipo:'nacional'},
    {dia:25,mes:1,descricao:'Anivers�rio de S�o Paulo',tipo:'estadual'},
    {dia:21,mes:4,descricao:'Tiradentes',tipo:'nacional'},
    {dia:1,mes:5,descricao:'Dia do Trabalho',tipo:'nacional'},
    {dia:9,mes:7,descricao:'Revolu��o Constitucionalista',tipo:'estadual'},
    {dia:7,mes:9,descricao:'Independ�ncia do Brasil',tipo:'nacional'},
    {dia:12,mes:10,descricao:'Nossa Sra. Aparecida',tipo:'nacional'},
    {dia:2,mes:11,descricao:'Finados',tipo:'nacional'},
    {dia:15,mes:11,descricao:'Proclama��o da Rep�blica',tipo:'nacional'},
    {dia:25,mes:12,descricao:'Natal',tipo:'nacional'},
    {dia:ad(p,-48).getDate(),mes:ad(p,-48).getMonth()+1,descricao:'Carnaval (2�)',tipo:'nacional'},
    {dia:ad(p,-47).getDate(),mes:ad(p,-47).getMonth()+1,descricao:'Carnaval (3�)',tipo:'nacional'},
...
export default function Encargos() {
  ...
  const salvarFuncionario = async () => {
    ...
    try {
      ...
      await registrarLog({ acao: form.id ? 'EDITAR' : 'INCLUIR', modulo: 'encargos', descricao: (form.id ? 'Funcion�rio atualizado: ' : 'Funcion�rio criado: ') + form.nome })
      showNotif('Funcion�rio salvo!')
  ...
  const feriadosFixosMes = [
    ...
    {dia:15,mes:11,descricao:'Proclama��o da Rep�blica',tipo:'nacional'},
    {dia:25,mes:12,descricao:'Natal',tipo:'nacional'},
    {dia:_add(_pasc,-48).getDate(),mes:_add(_pasc,-48).getMonth()+1,descricao:'Carnaval (2�)',tipo:'nacional'},
    {dia:_add(_pasc,-47).getDate(),mes:_add(_pasc,-47).getMonth()+1,descricao:'Carnaval (3�)',tipo:'nacional'},
    ...
    <div style={{ padding: '24px 32px', color: '#E8EAF0', maxWidth: 1200 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 12, color: '#7B82A0', marginLeft: 8 }}>Dias �teis: <b style={{ color: '#E8EAF0' }}>{diasUteis}</b></span>
          <span style={{ fontSize: 12, color: '#7B82A0', marginLeft: 16, borderLeft: '1px solid #252836', paddingLeft: 16 }}>
            ...
            <span style={{ marginLeft: 6 }}>Dias �teis: <b style={{ color: '#E8EAF0' }}>{diasUteisProx}</b></span>
          </span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[{ label: 'Total Sal�rios', valor: totalSalarios, cor: '#4F8EF7' }, { label: 'Total Encargos', valor: totalEncargosGeral, cor: '#FBBF24' }, { label: 'Custo Total Empresa', valor: totalGeral, cor: '#34D399' }, { label: 'Dep�sito (F�r+13�+FGTS+Multa)', valor: totalDeposito, cor: '#A78BFA' }].map(c => (
          <div key={c.label} style={{ ...st.card, borderColor: c.cor + '44' }}>
            ...
            <div style={{ fontSize: 11, color: '#7B82A0', marginTop: 4 }}>{MESES[mesRef.mes - 1]}/{mesRef.ano} � {funcionarios.length} funcion�rio(s)</div>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {[['resumo', '?? Resumo Mensal'], ['funcionarios', '?? Funcion�rios'], ['feriados', '??? Feriados']].map(([k, l]) => (
        ...
        <div>
          ...
          <div style={{...st.card, borderColor:'#4F8EF744', overflowX:'auto'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead>
                <tr>{['Funcion�rio','Sal�rio','F�r+13�','FGTS','Multa40%','VT','VA','Sal.Din','HE(h)','%HE','H.Ext','DSR','Total Enc','%Enc','Custo'].map(h=><th key={h} style={st.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {calculos.map(f=>(
          ...
          <div style={{...st.card, borderColor:'#F8717144'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead>
                <tr>{['Funcion�rio','Cargo','INSS','Desc. VT','Faltas/Atr.','Vale 40%','Total Desc.'].map(h=><th key={h} style={st.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {calculos.map(f=>(
                  <tr key={f.id} onMouseEnter={e=>(e.currentTarget.style.background='#1A1D2A')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
      ...
      {aba === 'funcionarios' && (
        <div>
          <div style={{display:'flex',justifyContent:'flex-end',marginBottom:12}}>
            <button style={st.btn('#34D399')} onClick={()=>{setEditando('novo');setForm({empresa_id:1,vale_alimentacao:250,vale_transporte:true,salario_dinheiro:0})}}>+ Novo Funcion�rio</button>
          </div>
          {editando && (
            <div style={{...st.card,borderColor:'#4F8EF7',marginBottom:16}}>
              <div style={{fontSize:14,fontWeight:700,marginBottom:16}}>{form.id?'Editar':'Novo'} Funcion�rio</div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:16}}>
                {[{k:'nome',l:'Nome',t:'text'},{k:'cargo',l:'Cargo',t:'text'},{k:'salario_base',l:'Sal�rio Base (R$)',t:'text'},{k:'vale_alimentacao',l:'Vale Alimenta��o (R$)',t:'text'},{k:'salario_dinheiro',l:'Sal. em Dinheiro (R$)',t:'text'},{k:'empresa_id',l:'Empresa (1=SIX, 2=ENOVA)',t:'text'},{k:'vale_transporte_valor',l:'Valor Unit�rio VT (R$/dia)',t:'text'}].map(({k,l,t})=>(
                ...
                <div><div style={st.label}>Vale Transporte</div>
                  <select value={form.vale_transporte?'sim':'nao'} onChange={e=>setForm((p:any)=>({...p,vale_transporte:e.target.value==='sim'}))} style={st.input}>
                    <option value="sim">Sim (desconta 6%)</option><option value="nao">N�o</option>
                  </select>
                </div>
              </div>
          ...
          <div style={st.card}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr>{['Nome','Cargo','Empresa','Sal�rio Base','V.Alimenta��o','V.Transporte','Sal.Dinheiro','A��es'].map(h=><th key={h} style={st.th}>{h}</th>)}</tr></thead>
              <tbody>
                {funcionarios.map(f=>(
                  <tr key={f.id} onMouseEnter={e=>(e.currentTarget.style.background='#1A1D2A')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                    ...
                    <td style={{...st.td,color:f.vale_transporte?'#34D399':'#7B82A0'}}>{f.vale_transporte?'Sim':'N�o'}</td>
                    <td style={st.td}>{f.salario_dinheiro>0?fmtR(f.salario_dinheiro):'�'}</td>
                    <td style={st.td}><div style={{display:'flex',gap:6}}>
                      <button style={st.btn('#4F8EF7')} onClick={()=>{setEditando(f.id);setForm({...f})}}>??</button>
                      <button style={st.btn('#F87171')} onClick={()=>excluir(f.id,f.nome)}>???</button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
      ...
      {aba === 'feriados' && (
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
          <div style={{background:'#13151F',border:'1px solid #4F8EF744',borderRadius:10,padding:20}}>
            <div style={{fontSize:13,fontWeight:700,color:'#4F8EF7',marginBottom:16}}>{editandoFeriado ? '?? Editando Feriado' : '? Adicionar Feriado'}</div>
            <div style={{display:'grid',gridTemplateColumns:'80px 1fr',gap:10,marginBottom:10}}>
              ...
              <div><div style={{fontSize:11,fontWeight:600,color:'#7B82A0',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>M�s</div>
                <select value={formFeriado.mes} onChange={e=>setFormFeriado(p=>({...p,mes:e.target.value}))} style={{background:'#0E1017',border:'1px solid #2A2D3E',borderRadius:6,color:'#E8EAF0',padding:'7px 10px',fontSize:13,width:'100%',outline:'none'}}>
                  {['Janeiro','Fevereiro','Mar�o','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'].map((m,i)=><option key={i} value={i+1}>{m}</option>)}
                </select></div>
            </div>
            <div style={{marginBottom:10}}><div style={{fontSize:11,fontWeight:600,color:'#7B82A0',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Descri��o</div>
              <input type='text' placeholder='Ex: Anivers�rio de S�o Paulo' value={formFeriado.descricao} onChange={e=>setFormFeriado(p=>({...p,descricao:e.target.value}))} style={{background:'#0E1017',border:'1px solid #2A2D3E',borderRadius:6,color:'#E8EAF0',padding:'7px 10px',fontSize:13,width:'100%',outline:'none'}}/>
            </div>
            <div style={{marginBottom:16}}><div style={{fontSize:11,fontWeight:600,color:'#7B82A0',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:6}}>Tipo</div>
              <select value={formFeriado.tipo} onChange={e=>setFormFeriado(p=>({...p,tipo:e.target.value}))} style={{background:'#0E1017',border:'1px solid #2A2D3E',borderRadius:6,color:'#E8EAF0',padding:'7px 10px',fontSize:13,width:'100%',outline:'none'}}>
                ...
                <option value='estadual'>??? Estadual (SP)</option>
                <option value='municipal'>??? Municipal (S�o Paulo)</option>
              </select>
            </div>
            <button onClick={salvarFeriado} style={{background:'#4F8EF7',color:'#fff',border:'1px solid #4F8EF7',borderRadius:6,padding:'8px 20px',fontSize:13,fontWeight:600,cursor:'pointer',width:'100%'}}>{editandoFeriado ? '?? Salvar Altera��es' : '?? Salvar Feriado'}</button>
            <div style={{marginTop:16,padding:'10px 12px',background:'#0E1017',borderRadius:6,border:'1px solid #2A2D3E'}}>
              <div style={{fontSize:11,color:'#7B82A0',lineHeight:1.6}}>?? Somados aos feriados fixos j� inclu�dos automaticamente (Carnaval, P�scoa, Corpus Christi, etc).</div>
            </div>
          </div>
          <div style={{background:'#13151F',border:'1px solid #252836',borderRadius:10,padding:20}}>
            <div style={{fontSize:13,fontWeight:700,color:'#E8EAF0',marginBottom:16}}>?? Feriados Cadastrados <span style={{fontSize:11,color:'#7B82A0',fontWeight:400}}>({feriadosFixosMes.length + feriadosCustom.length} no m�s, {feriadosCustom.length} personalizados)</span></div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
                ...
                <div key={'fx'+i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0E101780',borderRadius:6,padding:'8px 12px',border:'1px solid #2A2D3E88',opacity:0.85}}>
                  <div style={{display:'flex',alignItems:'center',gap:10}}>
                    <span style={{fontSize:14}}>{f.tipo==='nacional'?'????':'???'}</span>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:'#A0A8C0'}}>{f.descricao}</div>
                      <div style={{fontSize:11,color:'#5A6080'}}>{String(f.dia).padStart(2,'0')}/{String(f.mes).padStart(2,'0')} � <span style={{color:'#5A6080'}}>{f.tipo} � fixo autom�tico</span></div>
                    </div>
                  </div>
                  ...
                  <div key={f.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0E1017',borderRadius:6,padding:'8px 12px',border:'1px solid #2A2D3E'}}>
                    <div style={{display:'flex',alignItems:'center',gap:10}}>
                      <span style={{fontSize:16}}>{f.tipo==='nacional'?'????':f.tipo==='estadual'?'???':'???'}</span>
                      <div>
