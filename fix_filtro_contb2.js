...
// Verificacao rapida: os passos 1, 2, 4, 5, 6 do script anterior ja passaram na simulacao (nao foram salvos).
// Vamos reaplicar tudo do zero para garantir consistencia.
...
if (idx2 === -1) { console.log("FALHOU: anchor2"); ok = false; }
  ...
  const insertPoint = idx2 + anchor2.indexOf("</select>") + "</select>".length;
  c = c.slice(0, insertPoint) + novoSelect + c.slice(insertPoint);
  console.log("OK: select adicionado");
}

// 3. notasFiltradas5 + trocar notasFiltradas4.map por notasFiltradas5.map (quebra de linha \n, nao \r\n)
