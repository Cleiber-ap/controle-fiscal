...
const ui =
  ...
  "            <thead><tr style={{borderBottom:'1px solid #252836'}}>{['RB 12 meses (R$)','Al\u00EDquota','Dedu\u00E7\u00E3o (R$)','Faixa','ICMS'].map(h=>(<th key={h} style={{padding:'7px 10px',textAlign:h==='RB 12 meses (R$)'?'left':'right' as any,fontSize:'10px',fontWeight:600,color:'#4A5070',textTransform:'uppercase' as any,letterSpacing:'0.08em'}}>{h}</th>))}</tr></thead>\n" +
  "            <tbody>{FAIXAS_SIMPLES.map((f,i)=>{ const ativa=f.faixa===faixaAtual.faixa; return (<tr key={i} style={{background:ativa?'rgba(79,142,247,0.08)':'transparent',borderBottom:'1px solid #1A1D2A'}}><td style={{padding:'7px 10px',color:ativa?'#4F8EF7':'#E8EAF0',fontWeight:ativa?600:400,...mono,fontSize:'11px'}}>{f.min.toLocaleString('pt-BR',{minimumFractionDigits:2})} \u2013 {f.max.toLocaleString('pt-BR',{minimumFractionDigits:2})}</td><td style={{padding:'7px 10px',textAlign:'right' as any,color:ativa?'#4F8EF7':'#7B82A0',fontWeight:ativa?600:400,...mono}}>{(f.aliq*100).toFixed(2).replace('.',',')}%</td><td style={{padding:'7px 10px',textAlign:'right' as any,color:ativa?'#4F8EF7':'#7B82A0',...mono}}>{f.ded.toLocaleString('pt-BR',{minimumFractionDigits:2})}</td><td style={{padding:'7px 10px',textAlign:'right' as any}}><span style={{background:ativa?'rgba(79,142,247,0.2)':'transparent',color:ativa?'#4F8EF7':'#7B82A0',padding:ativa?'2px 7px':'0',borderRadius:'4px',fontWeight:ativa?600:400}}>{f.faixa}{ativa?' \u2605':''}</span></td><td style={{padding:'7px 10px',textAlign:'right' as any,color:ativa?'#4F8EF7':'#7B82A0',...mono}}>{f.icms>0?(f.icms*100).toFixed(2).replace('.',',')+' %':'\u2014'}</td></tr>)})}</tbody>\n" +
  "          </table>\n" +
  "        </div>\n" +
  ...
  "          {[{label:'Al\u00EDquota nominal',value:(faixaAtual.aliq*100).toFixed(2).replace('.',',')+' %'},{label:'Parcela a deduzir',value:'R$ '+faixaAtual.ded.toLocaleString('pt-BR',{minimumFractionDigits:2})}].map((row,i)=>(<div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:'6px'}}><span style={{fontSize:'11px',color:'#7B82A0'}}>{row.label}</span><span style={{fontSize:'12px',fontWeight:600,color:'#E8EAF0',...mono}}>{row.value}</span></div>))}\n" +
  "          <div style={{height:'1px',background:'#252836',margin:'10px 0'}} />\n" +
  "          <div style={{fontSize:'10px',color:'#4A5070',marginBottom:'8px',fontStyle:'italic'}}>F\u00F3rmula: (RBT12 \u00D7 al\u00EDquota \u2212 PD) / RBT12</div>\n" +
  "          {[{label:'RBT12 \u00D7 '+(faixaAtual.aliq*100).toFixed(2)+'%',value:fmtR(rbt12*faixaAtual.aliq)},{label:'\u2212 Dedu\u00E7\u00E3o',value:fmtR(rbt12*faixaAtual.aliq-faixaAtual.ded)},{label:'Al\u00EDquota efetiva',value:(aliqEfetiva*100).toFixed(2).replace('.',',')+' %',destaque:true}].map((row:any,i)=>(<div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:'6px'}}><span style={{fontSize:'11px',color:'#7B82A0'}}>{row.label}</span><span style={{fontSize:row.destaque?'15px':'12px',fontWeight:600,color:row.destaque?'#34D399':'#E8EAF0',...mono}}>{row.value}</span></div>))}\n" +
  "          <div style={{height:'1px',background:'#252836',margin:'10px 0'}} />\n" +
...
console.log('Ancora encontrada na posicao:', pos, 'de', c.length);
if (pos !== -1) {
  c = c.slice(0, pos) + ui + '\n' + anchor;
  fs.writeFileSync(p, c, 'utf8');
  console.log('OK - linhas:', c.split('\n').length);
} else {
  console.log('FALHOU - ancora nao encontrada');
  console.log('Fim do arquivo:', JSON.stringify(c.slice(-50)));
}
