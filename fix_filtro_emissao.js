...
// 2. Aplicar filtro de emissão na lista notasFiltradas (após filtroMesPagto)
...
// 4. Atualizar contador de notas no header
c = c.replace(
  '>{notasFiltradas.length} notas',
  '>{notasFiltradas2.length} notas'
);

// 5. Adicionar select de emissão ao lado do select de pagamento
const selectPagtoEnd = '</select>\n            </div>\n        </div>';
const selectEmissaoAdd = `</select>
...
if (!c.includes(selectPagtoEnd)) { console.log('NAO ENCONTRADO: select end'); process.exit(1); }
c = c.replace(selectPagtoEnd, selectEmissaoAdd);
